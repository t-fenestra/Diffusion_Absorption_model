...
pathFiles='/Users/pichugina/Work/Diffusion_Absorption_model/Control_without_growth/D_0.1/'
setwd(pathFiles)
library(tidyverse)
library(reshape2)
...
datastamp=dataStamps[4]

##########################################################################
# One Stype_profile
Stype_profile_file=paste(datastamp,"__Stype_profile.txt",sep="")
Stype_profile=as.data.frame(read.csv(Stype_profile_file,sep='\t',header=F))
Nlayers=dim(Stype_profile)[1]
Stype_profile=cbind(c(1:Nlayers),Stype_profile)
colnames(Stype_profile)[1] = "index"
df.melted <- melt(Stype_profile, id = "index")

#png("S_profile.png",width=800, height=500,res=200)
ggplot(data = df.melted, aes(x = index, y = value, color = variable)) +
  geom_line(show.legend=F)+ggtitle("Stype_profile") + xlab("Layers") + ylab("Stype concentration")+theme_bw()+theme(legend.position="none")
